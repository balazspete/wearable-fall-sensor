...
//		m.loudness;
		...
		while (check) {
			...
			leafA = treeA.node.getLeaf(HashMapAttributes.create("accVector", acc1));
			leafO = treeO.node.getLeaf(HashMapAttributes.create("accVector", ori1));
			leafL = treeL.node.getLeaf(HashMapAttributes.create("dbValue", ld1 ));
